...
const LaunchRequestHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = `<speak> 
            <voice name="Matthew">
                Hello! <break strength="weak"/> this is your Weather assistant.
        ...
        return await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${apiKey}&units=metric`)
              .then((response)=>{
                 ...
                 const speakOutput = 
                  ...
                  return handlerInput.responseBuilder
                        .speak(speakOutput)
                        .withStandardCard(`In ${weatherData.name}`, `its ${weatherData.weather[0].description} with ${weatherData.main.temp} degree centigrade. 
                                you can expect high at ${weatherData.main.temp_max} degrees and low at ${weatherData.main.temp_min} degrees.`,'https://ssl.gstatic.com/onebox/weather/48/sunny.png')
                        .getResponse();
              })
              .catch((error)=>{
